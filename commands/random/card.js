...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(color=='BLACK') {
        rankemoji = interaction.client.emojis.cache.filter(emoji => emoji.name === "b"+rank.toString()).first();
    } else {
        rankemoji = interaction.client.emojis.cache.filter(emoji => emoji.name === "a"+rank.toString()).first();
    }
    interaction.reply(
        `${rankemoji}\n${suitemoji}`
    );
